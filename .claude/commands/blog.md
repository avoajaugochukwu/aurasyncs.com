...
- @protocols/blog/blog-os-master.md — core philosophy, the Notion output contract, the accuracy & trust gate, anti-AI-slop checklist, the mandatory re-audit
- @protocols/blog/page-structures-skill.md — the 4 content types (💫 collection · 📅 daily/occasion · 🙏 faith/scripture · 🧘 practice guide), property shapes, body skeletons, word counts
...
Key facts to hold:
- **Output is Notion-native.** Posts are authored in Notion → pulled into `content/posts/<slug>.json` by `node --env-file=.env scripts/migrate-notion.mjs` → rendered by `components/NotionRenderer.tsx`. The page H1 comes from the **Title** property; the body uses only supported Notion blocks (the `quote` block is the answer box, `callout` is the tip). No tables, no math, no H1 in the body.
