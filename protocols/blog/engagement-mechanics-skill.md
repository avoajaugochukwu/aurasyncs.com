...
description: Scroll-depth psychology and scannability mechanics for blog posts. This is how an AuraSyncs affirmation post keeps a skimming reader scrolling and a real reader engaged — through scannability cadence, the But/Therefore rule, the Dopamine Ladder adapted for text, and the four web-specific retention killers. Calibrated for the Markdown/MDX renderer: the scannability events are the leading blockquote answer box, the affirmation lists themselves, themed sub-headings, bold-lead-in tips, and inline images.
---
...
This is the psychology of retention, mapped onto a page built from plain-Markdown MDX and rendered through `components/MdxContent.tsx` (`next-mdx-remote/rsc` + `remark-gfm`) with a component map styling `h1,h2,h3,p,ul,ol,li,blockquote,hr,code,pre,a,img`. Same psychology, different surface. GFM is on, so tables, strikethrough, and task lists render — use them sparingly; prose and lists usually serve an affirmation post better.
...
**For blogs:** the H1 (rendered from the frontmatter `title` in `app/blog/[slug]/page.tsx` — so there's no `#` H1 in the body) + featured image + the visible first lines are the stun gun. If those don't earn 2 more seconds, the reader bounces. On an "Affirmations for Anxiety" post, the calm featured image carries half the work before a single word is read.
...
**For blogs:** every `##` heading must promise something specific. "Background" is not a promise. "Affirmations for the 2am spiral, when sleep won't come" is.
...
**What it looks like:**
> 1,200 words of unbroken prose about why affirmations work — no sub-headings, no affirmation list, no bold-lead-in tip, no inline image, no blockquote.
...
**The fix:** every 200-300 words gets a *scannability event* — a themed sub-head (`##`/`###`), a bulleted or numbered affirmation list, a bold-lead-in tip, an inline image, or a blockquote. If you need to group affirmations, use sub-headings + lists rather than a grid (a table renders, but prose and lists read better here).
...
**The fix:** the opening blockquote IS the direct answer — "These 30 money affirmations reframe how you think about earning, saving, and abundance. Say a few out loud each morning, or pick one to carry through the day. They're a mindset practice, not a magic spell — here's how to make them land." 40-60 words. Then the first `##` heading with a section worth scrolling for.
...
For a blog: never let "Background" or "The Science of Affirmations" be the first `##` heading. Lead with the affirmations the reader can use right now; backfill the sourced "why they work" section only when the reader is already invested.
...
> [next `##` heading shows the daily-practice set]
...
The rule is one **scannability event every 200-300 words**. On this site the events are the ones that *actually render* through `MdxContent.tsx`:

- **Sub-head** (`##` or `###`) — a themed affirmation group
- **Bulleted or numbered list** — the affirmation lines themselves, the strongest event on an affirmation post
- **A bold-lead-in tip** — a how-to-use tip or gentle note led with a **bold phrase** (or a short blockquote)
- **An inline image** — a calming or thematic image, `![alt](/blog/<slug>-content-N.webp)`
- **A blockquote** — the opening answer box, or a single highlighted affirmation / cited line

Notes on the surface:

- **Custom callout components don't exist** — there's no JSX component map beyond the styled HTML elements. A tip is a **bold-lead-in line** or a **blockquote**; the answer box is a leading **blockquote**.
- **Tables render** (remark-gfm) but rarely earn their place here — to group affirmations, prefer themed sub-headings with lists underneath; reach for a grid only when the data is genuinely tabular.

For affirmation sets, the leading blockquote answer and the themed affirmation lists are the strongest events — they break the prose visually and reward the skimmer exactly when they're scanning for "just give me the affirmations." A set grouped into five themed sub-sections is five visual beats. For practice guides, each method step or example affirmation is its own beat.
...
For each band, count only events that render here (blockquote answer, affirmation lists, themed sub-heads, inline images, bold-lead-in tips):
...
(That table above is in this skill *doc*. GFM tables do render in a published post — but use them sparingly; for affirmation cadence, sub-heads and lists almost always read better than a grid.)
...
3. **Skim the `##` heading list** (5 seconds)
...
- A scannability event (affirmation list, inline image, bold-lead-in tip)
...
- **Reaching for a table where a list would serve** — a grid of affirmations reads worse than themed sub-heads + lists, even though tables render
...
- [ ] A scannability event every 200-300 words (events that render: blockquote, affirmation lists, bold-lead-in tips, inline images, sub-heads — tables sparingly)
